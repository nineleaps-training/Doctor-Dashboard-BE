...
public interface DoctorRepository extends PagingAndSortingRepository<DoctorDetails, Long> {
    ...
    @Query(value = "select new com.dashboard.doctor_dashboard.dtos.DoctorDropdownDto(dd.id,ld.name,ld.emailId,dd.speciality) from DoctorDetails dd inner join LoginDetails ld on deleted = false and  dd.loginId=ld.id")
